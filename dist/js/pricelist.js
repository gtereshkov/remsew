(function ($) {
    $.fn.pricelistTabs = function (userParams) {
        ...
        return $(this).each(function () {
            ...
            // Добавляем класс pricelist-tabs корневому блоку
            tabs.addClass('pricelist-tabs');
            // Сохраняем все имена вкладок в массив
            tabs.children().each(function () {
                tabsTitlesNames.push($(this).attr('data-title'));
            }).addClass('pricelist-tab');
            // Между корневым div и его содержимым добавляем div с классом "pricelist-tabs-content"
            tabs.wrapInner('<div class="pricelist-tabs-content"></div>');
            // В начало корневого div добавляем div с классом "pricelist-tabs-titles" и списком внутри
            tabs.prepend('<div class="pricelist-tabs-titles"><ul></ul></div>');
            // Помещаем в переменные:
            var tabsTitles = tabs.find('.pricelist-tabs-titles'), // Div c заголовками вкладок
                tabsContent = tabs.find('.pricelist-tabs-content'), // Div с содержимым вкладок
                tabsContentTabs = tabsContent.find('.pricelist-tab'); // Набор вкладок
